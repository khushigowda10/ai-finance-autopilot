...
const Insights = () => {
  ...
  transactions.forEach((t) => {
    if (!isIncome(t)) {
      categoryMap[t.category] =
        (categoryMap[t.category] || 0) + parseAmount(t.amount);
    }
  });
  ...
  budgets.forEach((b) => {
    ...
    if (percent > 100) {
      insights.push({
        type: "warning",
        text: `🚨 ${b.category}: Overspent ₹${spent - budget}`,
      });
    } else if (percent > 80) {
      insights.push({
        type: "warning",
      ...
      insights.push({
        type: "positive",
        text: `✅ ${b.category}: Good (${percent.toFixed(0)}%)`,
      });
    }
  });
  ...
  if (savingsRate > 30) {
    insights.push({
      type: "positive",
      text: `💰 Saving ${savingsRate.toFixed(1)}% — great job!`,
    });
  }
  ...
  if (topCategory) {
    insights.push({
      type: "neutral",
      text: `🔥 Top spending: ${topCategory[0]} (₹${topCategory[1]})`,
    });
  }
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-gray-900 p-4 sm:p-6 rounded-2xl border border-gray-800 space-y-4">
          <h2 className="text-base sm:text-lg">Key Metrics</h2>
        ...
        <div className="space-y-3 sm:space-y-4">
          <h2 className="text-base sm:text-lg">Smart Insights</h2>
          ...
          {insights.length === 0 ? (
            ...
            insights.map((ins, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`p-3 sm:p-4 rounded-xl text-sm sm:text-base ${color}`}
                >
                  {ins.text}
                </div>
              );
